...
training_root = "../../dataset/quadrant1_data"
...
for one_sample in training_dataset.take(1):
        x_single, y_single = one_sample
        break

training_dataset = tf.data.Dataset.from_tensor_slices((tf.expand_dims(x_single, 0), tf.expand_dims(y_single, 0)))
training_dataset = training_dataset.batch(1)

validation_dataset = tf.data.Dataset.from_tensor_slices((tf.expand_dims(x_single, 0), tf.expand_dims(y_single, 0)))
...
history = mymodel.fit(x=training_dataset, validation_data=validation_dataset, epochs=epochs, steps_per_epoch=1,
    validation_steps=1, verbose=1, use_multiprocessing=False, workers=1, max_queue_size=5, callbacks=None,)
print(history)

# # Extract the final training and validation loss
train_loss = history.history['loss'][-1]
mymodel.save(f'saved_models/new_diff_cnn_wscheduler0.85_seed22222_lr{lr}_trainloss{train_loss:.5f}_epoch{epochs}.h5')
